...
module.exports = function(style, reference) {
    ...
    validate.scale = function(key, val, spec) {
        if (val.type !== undefined) val.type = val.type.toString();

        var specScale;
        if (!val.type || val.type.toString() === 'power') {
            if (val.rounding && val.rounding.toString() === 'floor') {
                specScale = reference['scale_power_floor'];
            } else {
                specScale = reference['scale_power_none'];
            }

        } else if (val.type === 'ordinal') {
            specScale = reference['scale_ordinal'];
        } else {
            return error(key, val, 'valid scale type expected, "%s" found', val.type);
        }

        if (!val.range || !val.domain) {
            return error(key, val, 'scale must have a domain and a range');
        }

        if (val.range.length !== val.domain.length) {
            return error(key, val, 'scale domain and range must have equal number of elements');
        }

        validate.object(key, val, specScale);

        if (specScale === reference['scale_power_none'] || specScale === reference['scale_power_floor']) {
            for (var i = 1; i < val.domain.length; i++) {
                if (val.domain[i] < val.domain[i - 1]) {
                    return error(key + '.domain', val.domain, 'domain elements must be in ascending order');
                }
            }
        }

        if (typeof val.range === 'object' && !Array.isArray(val.range)) {
            return error(key + '.range', val.range, 'nested scales not allowed');
        }
        ...
        if (specScale === reference['scale_power_none']) {
