...
module.exports = function(style, reference) {
    ...
    function validate(key, val, spec) {
        ...
        if (reference.$version < 8 && spec.function && type === 'object') {
            return validate['function-v7'](key, val, spec);
        }
    ...
    validate.function = function(key, val, spec) {
        var functionType = val.type ? unbundle(val.type) : 'exponential';

        // Check that the function type used is valid for the style property
        if (spec['function-output'] === 'discrete' && functionType === 'exponential') {
            error(key + '.type', functionType, 'function type must be "categorical" or "interval" for this style property');
        }

        // Run basic validations
        var functionSpec = extend({}, reference['function'], reference['function-' + functionType]);
        var errorsLength = errors.length;
        validate.object(key, val, functionSpec);
        if (errors.length > errorsLength) return;

        // Check that domain and range have a compatible number of elements
        if (functionType !== 'interval' && val.range.length !== val.domain.length) {
            error(key, val, 'domain and range must have equal number of elements');
        } else if (functionType === 'interval' && val.range.length !== val.domain.length + 1) {
            error(key, val, 'range must have one more element than domain');
        }
        ...
        if (functionType === 'exponential' || functionType === 'interval') {
            for (var i = 1; i < val.domain.length; i++) {
                if (val.domain[i] < val.domain[i - 1]) {
                    error(key + '.domain', val.domain, 'domain elements must be in ascending order');
                }
            }
        }
