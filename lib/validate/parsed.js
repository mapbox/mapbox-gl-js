...
module.exports = function(style, reference) {
    ...
    validate.scale = function(key, val, spec) {
        ...
        else val.type = 'exponential';

        // Run basic validations
        var scaleSpec = _.extend({}, reference['scale_' + val.type], reference['scale']);
        var errorsLength = errors.length;
        validate.object(key, val, scaleSpec);
        if (errors.length > errorsLength) return;

        // Check that domain and range have a compatible number of elements
        if (val.type !== 'interval' && val.range.length !== val.domain.length) {
            error(key, val, 'domain and range must have equal number of elements');
            return;
        } else if (val.type === 'interval' && val.range.length !== val.domain.length + 1) {
            error(key, val, 'range must have one more element than domain');
            return;
        }

        // Check that domain is in ascending order
        if (val.type === 'exponential' || val.type === 'interval') {
