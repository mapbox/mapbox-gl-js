...
module.exports = function(style, reference) {
    ...
    function validate(key, val, spec) {
        ...
        // Functions
        if (reference.$version < 8 && spec.function && type === 'object') {
            return validate.function(key, val, spec);
        }

        // Scales
        if (reference.$version >= 8 && spec['scale-output'] && type === 'object') {
            return validate.scale(key, val, spec);
        }
    ...
    validate.scale = function(key, scale, spec) {
        var scaleType = scale.type ? unbundle(scale.type) : 'exponential';

        // Check that the scale type used is valid for the style property
        if (spec['scale-output'] === 'discrete' && scaleType === 'exponential') {
            error(key + '.type', scaleType, 'scale type must be "categorical" or "interval" for this style property');
        }

        // Run basic validations
        var scaleSpec = extend({}, reference['scale'], reference['scale_' + scaleType]);
        var errorsLength = errors.length;
        validate.object(key, scale, scaleSpec);
        if (errors.length > errorsLength) return;

        // Check that domain and range have a compatible number of elements
        if (scaleType !== 'interval' && scale.range.length !== scale.domain.length) {
            error(key, scale, 'domain and range must have equal number of elements');
        } else if (scaleType === 'interval' && scale.range.length !== scale.domain.length + 1) {
            error(key, scale, 'range must have one more element than domain');
        }

        // Check that domain is in ascending order
        if (scaleType === 'exponential' || scaleType === 'interval') {
            for (var i = 1; i < scale.domain.length; i++) {
                if (scale.domain[i] < scale.domain[i - 1]) {
                    error(key + '.domain', scale.domain, 'domain elements must be in ascending order');
                }
            }
        }

        // Ensure all range values are of the correct type
        validate.array(key + '.range', scale.range, {type: "array", value: spec});

        // Ensure all range values are of the same type and interpolate-able
        if (scaleType === 'exponential') {
            var type = typeof_(scale.range[0]);

            if (type === 'array') {
                var length = scale.range[0].length;
                validate.array(key + '.range', scale.range, {type: "array", value: {type: "array", value: "number", length: length}});
            } else {
                validate.array(key + '.range', scale.range, {type: "array", value: "number"});
            }
