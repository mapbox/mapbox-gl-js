...
module.exports.validate.style = function(property, value, line, constants) {
    ...
    var err = checkType(property, value, ref, line, constants);
    if (err) return err;
    else return [];
};

module.exports.validate.bucket = function(bucket) {
    if (typeof bucket.filter !== 'object') {
        return [{
            message: 'bucket filter property required',
            line: bucket.__line__
        }];
    }
    return [];
};

function checkType(property, value, ref, line, constants) {
    if (ref.function && typeof value === 'object') {
        ...
        return checkType(property, constants[value], ref, line);
