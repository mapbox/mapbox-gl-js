...
module.exports = function(str) {
    ...
    function validate(key, val, spec) {
        ...
        // Requires
        if (spec.requires) {
            var layer = rawStyle.layers[key.split('.')[0].slice(7, -1)];
            spec.requires.forEach(function(property) {
                validate.requires(layer, property, key, val);
            });
        }
    ...
    validate.requires = function(layer, property, key, val) {
        if (typeof property === 'string') {
            if (!(layer.layout && layer.layout[property]) &&
                !layer.paint[property]) return error(key, val, 'property requires "%s"', property);
        } else {
            if (property['!']) {
                if ((layer.layout && layer.layout[property['!']]) ||
                    layer.paint[property['!']]) return error(key, val, 'property is disabled by "%s"', property['!']);
            } else {
                var requiredProp = Object.keys(property)[0];
                var requiredVal = property[requiredProp];
                if ((layer.layout && layer.layout[requiredProp] !== requiredVal) &&
                    layer.paint[requiredProp] !== requiredVal) return error(key, val, 'property requires "%s" to be "%s"', requiredProp, requiredVal);
            }
        }
    };
    ...
    validate.layer = function(key, val) {
        ...
        if ('ref' in val) {
            ...
            if (!val.source) {
                ...
                } else if (source.type == 'vector' && type == 'raster' && !val.layers) {
        ...
        validate.object(key, val, reference.layer, {
            ...
            layout: function(key, val) {
                return type && validate(key, val, reference['layout_' + type]);
            },
            paint: function(key, val) {
                return type && validate(key, val, reference['paint_' + type]);
            }
        });
    };
    ...
    validate.array = function (key, val, spec, validator) {
        ...
        for (var i = 0; i < val.length; i++) {
            (validator || validate)(key + '[' + i + ']', val[i], {
                "type": spec.value,
