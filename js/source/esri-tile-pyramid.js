...
var Tile = require('mapbox-gl/js/source/tile');
var TileCoord = require('mapbox-gl/js/source/tile_coord');
var Point = require('mapbox-gl/node_modules/point-geometry');
var Cache = require('mapbox-gl/js/util/mru_cache');
...
TilePyramid.prototype = {
  ...
  coveringTiles: function (transform) {
    ...
    /*//the added logic to not try to download empty tiles
    var indexed = tiles.map(function (t) {
      return this._indexSearch(t);
    }, this);

    tiles = [];
    indexed.forEach(function (id) {
      if (tiles.indexOf(id) === -1) {
        tiles.push(id);
      }
    });*/
  ...
  addTile: function (id) {
    ...
    if (!tile) {
      ...
      tile.parentId = this._indexSearch(id); // get the parent tile in the tile index
  ...
  _indexSearch: function (id) {
    ...
    id = TileCoord.toID(tile.z, tile.x, tile.y);
    ids.push(id);
    while (id !== 0) {
    ...
    while (ids.length) {
      id = ids.pop();
      index = TileCoord.children(cursorId).indexOf(id);
      if (cursor[index] === 0) {
        id = cursorId;
        break;
      }
    ...
    tile = TileCoord.fromID(id);
    return TileCoord.toID(tile.z, tile.x, tile.y, w);
  }
};
