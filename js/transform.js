...
Transform.prototype = {
    ...
    get lat() {
        var g = Math.exp((this.center[1] - this._hH) / this._Cc);
        return -360 / Math.PI * Math.atan(g) + 90;
    },

    set lat(lat) {
        var f = Math.min(Math.max(Math.sin((Math.PI / 180) * -lat), -0.9999), 0.9999);
        this.y = -(-this.world/2*Math.sqrt(2)*Math.sin(this.rotation-3*Math.PI/4) - 0.5 * Math.log((1 + f) / (1 - f)) * this._Cc) + this._hH;
    }
};
