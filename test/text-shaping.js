...
test('shaping', function(t) {
    ...
    shaped = shaping.shape('hi' + String.fromCharCode(0), name, stacks, 15 * oneEm, oneEm, 0.5, 0 * oneEm, [0, 0]);
    ...
    // Default shaping.
    shaped = shaping.shape('abcde', name, stacks, 15 * oneEm, oneEm, 0.5, 0 * oneEm, [0, 0]);
    ...
    // Letter spacing.
    shaped = shaping.shape('abcde', name, stacks, 15 * oneEm, oneEm, 0.5, 0.125 * oneEm, [0, 0]);
    ...
    // Line break.
    shaped = shaping.shape('abcde abcde', name, stacks, 4 * oneEm, oneEm, 0.5, 0 * oneEm, [0, 0]);
    ...
    // Null shaping.
    shaped = shaping.shape('', name, stacks, 15 * oneEm, oneEm, 0.5, 0 * oneEm, [0, 0]);
    t.equal(false, shaped);

    shaped = shaping.shape(String.fromCharCode(0), name, stacks, 15 * oneEm, oneEm, 0.5, 0 * oneEm, [0, 0]);
    t.equal(false, shaped);
