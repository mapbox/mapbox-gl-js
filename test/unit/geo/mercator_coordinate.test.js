...
test('LngLat', (t) => {
    ...
    t.test('#meterInMercatorCoordinateUnits', (t) => {
        const nullIsland = new LngLat(0, 0);
        t.equal(MercatorCoordinate.fromLngLat(nullIsland).meterInMercatorCoordinateUnits(), 2.4981155719964737e-8, 'length of 1 meter in MercatorCoordinate units at the equator');
        t.end();
    });
