...
describe("mapbox", () => {
  ...
  describe("RequestManager", () => {
    ...
    test("identifies expired tokens correctly", () => {
      ...
      withFixedDate(now + ms13Hours, () =>
        manager.normalizeTileURL("mapbox://tiles/a.b/0/0/0.pbf")
      );
    ...
    describe(".normalizeStyleURL", () => {
      test("returns an API URL with access_token parameter when no query string", () => {
        ...
        expect(
          manager.normalizeStyleURL("mapbox://styles/user/style/draft")
        ).toEqual(
          `https://api.mapbox.com/styles/v1/user/style/draft?sdk=js-${version}&access_token=key`
        );
      });

      test("returns an API URL with access_token parameter when query string exists", () => {
        expect(
          manager.normalizeStyleURL("mapbox://styles/user/style?fresh=true")
        ).toEqual(
          `https://api.mapbox.com/styles/v1/user/style?fresh=true&sdk=js-${version}&access_token=key`
        );
        expect(
          manager.normalizeStyleURL(
            "mapbox://styles/user/style/draft?fresh=true"
          )
        ).toEqual(
    ...
    describe(".normalizeSourceURL", () => {
      test("returns a v4 URL with access_token parameter", () => {
        expect(manager.normalizeSourceURL("mapbox://user.map")).toEqual(
          "https://api.mapbox.com/v4/user.map.json?secure&access_token=key"
        );
      });

      test("uses provided access token", () => {
        expect(
          manager.normalizeSourceURL("mapbox://user.map", "token")
        ).toEqual(
          "https://api.mapbox.com/v4/user.map.json?secure&access_token=token"
        );
      });

      test("uses provided query parameters", () => {
        expect(
          manager.normalizeSourceURL("mapbox://user.map?foo=bar", "token")
        ).toEqual(
      ...
      test("works with composite sources", () => {
        expect(
          manager.normalizeSourceURL("mapbox://one.a,two.b,three.c")
        ).toEqual(
      ...
      test("adds language query parameter", () => {
        expect(
          manager.normalizeSourceURL("mapbox://user.map?foo=bar", "token", "es")
        ).toEqual(
      ...
      test("adds worldview query parameter", () => {
        expect(
          manager.normalizeSourceURL(
            "mapbox://user.map?foo=bar",
            "token",
            null,
            "JP"
          )
      ...
      test("throws an error if no access token is provided", () => {
        ...
        expect(() => {
          manager.normalizeSourceURL("mapbox://user.map");
        }).toThrowError("An API access token is required to use Mapbox GL.");
        config.ACCESS_TOKEN = "key";
      });
      ...
      test("throws an error if a secret access token is provided", () => {
        ...
        expect(() => {
          manager.normalizeSourceURL("mapbox://user.map");
        }).toThrowError("Use a public access token (pk.*) with Mapbox GL");
        config.ACCESS_TOKEN = "key";
      });

      test("ignores non-mapbox:// scheme", () => {
        expect(manager.normalizeSourceURL("http://path")).toEqual(
          "http://path"
        );
      });
    ...
    describe(".normalizeGlyphsURL", () => {
      ...
      test("normalizes mapbox:// URLs when query string exists", () => {
        expect(
          manager.normalizeGlyphsURL(
            "mapbox://fonts/boxmap/{fontstack}/{range}.pbf?fresh=true"
          )
        ).toEqual(
      ...
      test("ignores non-mapbox:// scheme", () => {
        expect(manager.normalizeGlyphsURL("http://path")).toEqual(
          "http://path"
        );
      });
    ...
    describe(".normalizeSpriteURL", () => {
      test("normalizes mapbox:// URLs when no query string", () => {
        expect(
          manager.normalizeSpriteURL(
            "mapbox://sprites/mapbox/streets-v8",
            "",
            ".json"
          )
        ).toEqual(
        ...
        expect(
          manager.normalizeSpriteURL(
            "mapbox://sprites/mapbox/streets-v8/draft",
            "@2x",
            ".png"
          )
        ).toEqual(
      ...
      test("normalizes mapbox:// URLs when query string exists", () => {
        expect(
          manager.normalizeSpriteURL(
            "mapbox://sprites/mapbox/streets-v8?fresh=true",
            "",
            ".json"
          )
        ).toEqual(
        ...
        expect(
          manager.normalizeSpriteURL(
            "mapbox://sprites/mapbox/streets-v8?fresh=false",
            "@2x",
            ".png"
          )
        ).toEqual(
        ...
        expect(
          manager.normalizeSpriteURL(
            "mapbox://sprites/mapbox/streets-v8/draft?fresh=true",
            "@2x",
            ".png"
          )
        ).toEqual(
      ...
      test("concantenates path, ratio, and extension for non-mapbox:// scheme", () => {
        expect(
          manager.normalizeSpriteURL("http://www.foo.com/bar", "@2x", ".png")
        ).toEqual("http://www.foo.com/bar@2x.png");
      });

      test("concantenates path, ratio, and extension for file:/// scheme", () => {
        expect(
          manager.normalizeSpriteURL("file:///path/to/bar", "@2x", ".png")
        ).toEqual("file:///path/to/bar@2x.png");
      });

      test("normalizes non-mapbox:// scheme when query string exists", () => {
        expect(
          manager.normalizeSpriteURL(
            "http://www.foo.com/bar?fresh=true",
            "@2x",
            ".png"
          )
    ...
    test("canonicalize raster tileset", () => {
      ...
      expect(
        manager.canonicalizeTileset(tileset, "mapbox://mapbox.satellite")
      ).toStrictEqual(["mapbox://tiles/mapbox.satellite/{z}/{x}/{y}.png"]);
    });
    ...
    test("canonicalize vector tileset", () => {
      ...
      expect(
        manager.canonicalizeTileset(tileset, "mapbox://mapbox.streets")
      ).toStrictEqual(["mapbox://tiles/mapbox.streets/{z}/{x}/{y}.vector.pbf"]);
    });
    ...
    test(".canonicalizeTileURL", () => {
      ...
      expect(
        manager.canonicalizeTileURL(
          "http://api.mapbox.com/v4/a.b/{z}/{x}/{y}.vector.pbf?access_token=key",
          tileJSONURL
        )
      ).toEqual("mapbox://tiles/a.b/{z}/{x}/{y}.vector.pbf");
      expect(
        manager.canonicalizeTileURL(
          "https://api.mapbox.cn/v4/a.b/{z}/{x}/{y}.vector.pbf?access_token=key",
          tileJSONURL
        )
      ).toEqual("mapbox://tiles/a.b/{z}/{x}/{y}.vector.pbf");
      expect(
      ...
      expect(
        manager.canonicalizeTileURL(
          "http://api.mapbox.com/raster/v1/a.b/{z}/{x}/{y}.png?access_token=key",
          tileJSONURL
        )
      ).toEqual("mapbox://raster/a.b/{z}/{x}/{y}.png");
      expect(
        manager.canonicalizeTileURL(
          "http://api.mapbox.com/rasterarrays/v1/a.b/{z}/{x}/{y}.mrt?access_token=key",
          tileJSONURL
        )
      ...
      expect(manager.canonicalizeTileURL("http://api.mapbox.com/v4/")).toEqual(
        "http://api.mapbox.com/v4/"
      );
      expect(
        manager.canonicalizeTileURL("http://api.mapbox.com/v4/a.b/{z}/{x}/{y}.")
      ).toEqual("http://api.mapbox.com/v4/a.b/{z}/{x}/{y}.");
      expect(
        manager.canonicalizeTileURL(
    ...
    describe(".normalizeTileURL", () => {
      test(".normalizeTileURL does nothing on 1x devices", () => {
        ...
        webpSupported.supported = false;
        expect(manager.normalizeTileURL("mapbox://path.png/tile.png")).toEqual(
          `http://path.png/v4/tile.png`
        );
        expect(
          manager.normalizeTileURL("mapbox://path.png/tile.png32")
        ).toEqual(`http://path.png/v4/tile.png32`);
        expect(
          manager.normalizeTileURL("mapbox://path.png/tile.jpg70")
        ).toEqual(`http://path.png/v4/tile.jpg70`);
      });
      ...
      test(".normalizeTileURL inserts @2x if source requests it", () => {
        ...
        config.REQUIRE_ACCESS_TOKEN = false;
        expect(
          manager.normalizeTileURL("mapbox://path.png/tile.png", true)
        ).toEqual(`http://path.png/v4/tile@2x.png`);
        expect(
          manager.normalizeTileURL("mapbox://path.png/tile.png32", true)
        ).toEqual(`http://path.png/v4/tile@2x.png32`);
        expect(
          manager.normalizeTileURL("mapbox://path.png/tile.jpg70", true)
        ).toEqual(`http://path.png/v4/tile@2x.jpg70`);
        expect(
          manager.normalizeTileURL(
            "mapbox://path.png/tile.png?access_token=foo",
            true
          )
      ...
      test(".normalizeTileURL inserts @2x for 512 raster tiles on v4 of the api", () => {
        ...
        config.REQUIRE_ACCESS_TOKEN = false;
        expect(
          manager.normalizeTileURL("mapbox://path.png/tile.png", false, 256)
        ).toEqual(`http://path.png/v4/tile.png`);
        expect(
          manager.normalizeTileURL("mapbox://path.png/tile.png", false, 512)
        ).toEqual(`http://path.png/v4/tile@2x.png`);
        expect(
          manager.normalizeTileURL("mapbox://raster/a.b/0/0/0.png", false, 256)
        ).toEqual(`http://path.png/raster/v1/a.b/0/0/0.png`);
        expect(
          manager.normalizeTileURL("mapbox://raster/a.b/0/0/0.png", false, 512)
        ).toEqual(`http://path.png/raster/v1/a.b/0/0/0.png`);
      });
      ...
      test(".normalizeTileURL replaces img extension with webp on supporting devices", () => {
        ...
        config.REQUIRE_ACCESS_TOKEN = false;
        expect(manager.normalizeTileURL("mapbox://path.png/tile.png")).toEqual(
          `http://path.png/v4/tile.webp`
        );
        expect(
          manager.normalizeTileURL("mapbox://path.png/tile.png32")
        ).toEqual(`http://path.png/v4/tile.webp`);
        expect(
          manager.normalizeTileURL("mapbox://path.png/tile.jpg70")
        ).toEqual(`http://path.png/v4/tile.webp`);
        expect(
        ...
        ).toEqual(`http://path.png/v4/tile.webp?access_token=foo`);
        webpSupported.supported = false;
      });

      test(".normalizeTileURL ignores non-mapbox:// sources", () => {
        expect(manager.normalizeTileURL("http://path.png")).toEqual(
          "http://path.png"
        );
      });
      ...
      test(".normalizeTileURL does not modify the access token for non-mapbox sources", () => {
        ...
        expect(
          manager.normalizeTileURL(
            "http://example.com/tile.png?access_token=tk.abc.123"
          )
      ...
      test(".normalizeTileURL matches gl-native normalization", () => {
        ...
        expect(
          manager.normalizeTileURL(
            "mapbox://tiles/a.b/0/0/0.pbf?style=mapbox://styles/mapbox/streets-v9@0"
          )
        ).toEqual(
    ...
    test("POSTs cn event when API_URL change to cn endpoint", async () => {
      ...
      await new Promise((resolve) => {
        setTimeout(() => {
          ...
          expect(requestURL.origin).not.toEqual("https://events.mapbox.cn");
          resolve();
        }, 0);
      });
    });
    ...
    describe("with LocalStorage available", () => {
      ...
      test("does not POST event when previously stored data is on the same day", () => {
        ...
        window.localStorage.setItem(
          `mapbox.eventData.uuid:${config.ACCESS_TOKEN}`,
          uuid()
        );
        window.localStorage.setItem(
          `mapbox.eventData:${config.ACCESS_TOKEN}`,
          JSON.stringify({
            lastSuccess: now,
            tokenU: "key",
          })
      ...
      test("does not POST appuserTurnstile event second time within same calendar day", async () => {
        ...
        const reqBody = JSON.parse(await req.requestBody)[0];
        equalWithPrecision(
          new Date(reqBody.created).valueOf(),
          firstEvent,
          100
        );
      });
  ...
  describe("MapLoadEvent", () => {
    ...
    test("POSTs cn event when API_URL changes to cn endpoint", () => {
      ...
      expect(url.hostname).toContain("events.mapbox.cn");
    });
    ...
    describe("with LocalStorage available", () => {
      ...
      test("uses the same uuid as TurnstileEvent", async () => {
        ...
        const turnstileEventData = JSON.parse(
          window.localStorage.getItem(`mapbox.eventData:${config.ACCESS_TOKEN}`)
        );
    ...
    test("send a new request when access token changes", async () => {
      ...
      await new Promise((resolve) => {
        ...
        expect(req.url).toEqual(
          ...
          }?sku=${skuToken}&access_token=pk.new.*`
        );
