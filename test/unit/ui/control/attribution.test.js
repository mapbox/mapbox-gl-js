...
test('AttributionControl appears in bottom-right by default', (t) => {
    ...
    t.equal(map.getContainer().querySelectorAll('.mapbox-ctrl-bottom-right .mapboxgl-ctrl-attrib').length, 1);
    t.end();
});
...
test('AttributionControl has the correct edit map link', (t) => {
    ...
    map.addControl(attribution);
    map.on('load', () => {
        map.addSource('1', {type: 'vector', attribution: '<a class="mapboxgl-improve-map" href="https://www.mapbox.com/feedback/" target="_blank">Improve this map</a>'});
        map.on('data', (e) => {
            if (e.dataType === 'source' && e.sourceDataType === 'metadata') {
