        ...
        return (
            <div id={props.id} className='style-spec-section-h3'>
                <h3 className='pt60' style={{ marginTop: '-30px' }}>
                    <a className='unprose color-blue-on-hover' href={`#${props.id}`} title={`link to ${props.title}`}>
                        {props.title}</a>
                </h3>
                {props.children}
            </div>
        ...
        return (
            <div className='px12 px12 mb24'>
                ...
                <h4 className='pt60' style={{ marginTop: '-30px' }}>
                    <a className="unprose color-blue-on-hover" href={`#${props.id}`}>{props.title}</a>
                </h4>
                {props.children}
            </div>
        );
...
class InnerSection extends React.Component {
    render() {
        const {props} = this;
        return (
            <div className={`px24 py18 border border--gray-light round style-spec-inner-section ${props.className || props.className}`}>
                {this.props.children}
            </div>
        );
    }
...
class Item extends React.Component {
    ...
    render() {
        return (
            <div className='mb30'>
                <a style={{ marginTop: '-30px' }} className='pt60 unprose color-blue-on-hover txt-code bg-transparent mb3 txt-l block ml-neg3' href={`#${this.props.id}`}  id={this.props.id} >{this.props.name}</a>
                <div className='mb6'>
                    {this.props.kind === 'paint' &&
                    <em className='color-gray'><a href='#paint-property'>Paint</a> property. </em>}
                    {this.props.kind === 'layout' &&
                    <em className='color-gray'><a href='#layout-property'>Layout</a> property. </em>}
                ...
                <div className='style-spec-section prose'>
                    <div>
                        ...
                        <p>Developers using the <a href='https://www.mapbox.com/ios-sdk/'>Mapbox Maps SDK for iOS</a> or <a
                          ...
                          Mapbox Maps SDK for macOS</a> should consult the <a href='https://docs.mapbox.com/ios/maps/overview/'>iOS SDK API reference</a> for platform-appropriate
                                    ...
                                    <SectionH4 key={i} id={`expressions-${slug(group.name)}`} title={group.name} index={i} length={groupedExpressions.length}>
                                        ...
                                        {group.expressions.map(({name, doc, type, sdkSupport}, i) => (
                                            <div key={i}>
                                                ...
                                                <span className=''>
                                                    <a className='txt-code bg-transparent ml-neg6 unprose color-blue-on-hover txt-l'
                            ...
                            <InnerSection>
                                ...
                                <SectionH4 id="function-type" title='type'>
                                    ...
                                    <dl className='mb6'>
                                        ...
                                        <dd className='mb12'>A function that returns its input as the output.</dd>
                                ...
                                <SectionH4 id="function-colorSpace" title='colorSpace'>
                                    ...
                                    <dl className="">
                                        <dt><code>"rgb"</code></dt>
                                        <dd className='mb12'>Use the RGB color space to interpolate color values</dd>
                                        <dt><code>"lab"</code></dt>
                                        <dd className='mb12'>Use the LAB color space to interpolate color values.</dd>
                                        <dt><code>"hcl"</code></dt>
                                        <dd className='mb12'>Use the HCL color space to interpolate color values, interpolating
                    ...
                    <SectionH2 id='other-filter' title='Other filter'>
                        ...
                        <InnerSection>
                            ...
                            <SectionH4 title='Comparison Filters' id='comparison-filters'>
                                ...
                                <div className='mb6'>
                                    <code>["&gt;=", <var>key</var>, <var>value</var>]</code> <span className='txt-bold txt-s'>greater than or equal: <var>feature[key]</var> ≥ <var>value</var></span>
                                </div>
                                <div className='mb6'>
                                    <code>["&lt;", <var>key</var>, <var>value</var>]</code> <span className='txt-bold txt-s'>less than: <var>feature[key]</var> &lt; <var>value</var></span>
                                </div>
                                <div className='mb6'>
                                    <code>["&lt;=", <var>key</var>, <var>value</var>]</code> <span className='txt-bold txt-s'>less than or equal: <var>feature[key]</var> ≤ <var>value</var></span>
                                </div>
                            </SectionH4>

                            <SectionH4 title='Set Membership Filters' id='set-membership Filters-filters'>
                                <div className='mb6'>
                                    <code>["in", <var>key</var>, <var>v0</var>, ..., <var>vn</var>]</code> <span className='txt-bold txt-s'>set inclusion: <var>feature[key]</var> ∈ {`{`}<var>v0</var>, ..., <var>vn</var>{`}`}</span>
                                </div>
                                <div className='mb6'>
                                    <code>["!in", <var>key</var>, <var>v0</var>, ..., <var>vn</var>]</code> <span className='txt-bold txt-s'>set exclusion: <var>feature[key]</var> ∉ {`{`}<var>v0</var>, ..., <var>vn</var>{`}`}</span>
                                </div>
                            </SectionH4>

                            <SectionH4 title='Combining Filters' id='combining-filters'>
                                <div className='mb6'>
                                    <code>["all", <var>f0</var>, ..., <var>fn</var>]</code> <span className='txt-bold txt-s'>logical <code>AND</code>: <var>f0</var> ∧ ... ∧ <var>fn</var></span>
                                </div>
                                <div className='mb6'>
                                    <code>["any", <var>f0</var>, ..., <var>fn</var>]</code> <span className='txt-bold txt-s'>logical <code>OR</code>: <var>f0</var> ∨ ... ∨ <var>fn</var></span>
                                </div>
                                <div className='mb6'>
                                    <code>["none", <var>f0</var>, ..., <var>fn</var>]</code> <span className='txt-bold txt-s'>logical <code>NOR</code>: ¬<var>f0</var> ∧ ... ∧ ¬<var>fn</var></span>
                                </div>
                                <p>
