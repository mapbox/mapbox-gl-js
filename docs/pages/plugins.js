                ...
                <div>
                    <div id='plugins' className='doc' data-swiftype-index='true'>
                        {entries(plugins).map(([title, plugins], i) => <div key={i} className=''>
                            <a className={`unprose mb18 ${i > 0 ? 'pt60' : 'pt30'} block color-blue-on-hover`} href={`#${slug(title)}`} style={{marginTop: '-30px'}} id={slug(title)}><h2 className='txt-bold'>{title} ({entries(plugins).length})</h2></a>
                            {entries(plugins).map(([name, plugin], i) => <div key={i} className='mb18 border-b border--darken10'>

                                <div className='mb6'>{name}</div>
                                <div className='color-gray color-gray-on-hover mb6'>{ plugin.description }</div>
                                ...
                                <div className="pb18">
                                    ...
                                    { plugin.example &&
                                                <div className='mt6'>
                                                    <a className="link link--blue" href={prefixUrl(`/example/${plugin.example}`)}>
