...
module.exports = function validateLayer(options) {
    ...
    if ('ref' in layer) {
        ...
        if (!layer.source) {
            ...
            } else if (sourceType === 'raster-terrain' && type !== 'hillshade') {
                errors.push(new ValidationError(key, layer.source, 'raster-terrain source can only be used with layer type \'hillshade\'.', layer.id));
            }
        }
    }
