...
void main() {
    vec3 elevationVec = tileUpVector(a_anchor_pos / 8192.0) * elevation(a_anchor_pos);

    //vec4 projectedPoint = u_matrix * vec4(a_anchor_pos, elevation(a_anchor_pos), 1);
    //vec4 projectedPoint = u_matrix * vec4(a_anchor_pos, 1);

    vec4 projectedPoint = u_matrix * vec4(a_pos + elevationVec, 1);
