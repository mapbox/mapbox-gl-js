...
void main() {
    ...
    highp float a_lineangle = a_more_data[1] / 10000.0 * 2.0 * PI;
    //highp float a_lineangle = a_more_data[1];
    vec2 a_zoom = unpack_float(a_data[3]);
    highp float a_minzoom = a_zoom[0];
    highp float a_maxzoom = a_zoom[1];
    highp float a_anchorangle = a_more_data[0] / 10000.0 * 2.0 * PI;
    ...
    // rotation-alignment: map
        ...
        vec4 b = u_matrix * vec4(a_label_pos + vec2(cos(a_lineangle), sin(a_lineangle)), 0, 1);
        ...
        highp float asin = sin(a_lineangle);
        highp float acos = cos(a_lineangle);
    ...
    highp float perspective_zoom_adjust = log2(collision_perspective_ratio * collision_adjustment * legibility_expansion)*10.0 / 255.0;
