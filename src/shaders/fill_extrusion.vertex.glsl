...
void main() {
    ...
    // z is purpose of edge: 0: pattern, 1: in roofs, ratio of height
    mediump vec3 top_up_ny = a_pos_normal_ed.xyz - 2.0 * pos_nx;

    float zx = abs(pos_nx.z);
    float zbits = floor(zx / 128.0);
    float xbits = zx - 128.0 * zbits;
    float ybits = 126.0 - xbits - zbits;
    zbits /= (height - base);

    vec3 signs = sign(vec3(pos_nx.z, top_up_ny.z - 0.5, 1.0));
    vec3 normal = normalize(signs * vec3(xbits, ybits, zbits));
    ...
    float t = top_up_ny.x;
    if (top_up_ny.y == 1.0 && t <= 0.0) {
        t = 1.0;
        float zscale = a_pos_normal_ed.w / 8192.0;
        height = mix(base, height, zscale);        
    }
