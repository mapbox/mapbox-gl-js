...
class Camera
  ...
  _cameraForBoundsOnGlobe(
    ...
    options?: CameraOptions,
      ...
      const offsetDistance = frustumDistance + (tr.pitch === 0 ?
          0 :
  ...
  _cameraForBounds(
    ...
    options?: CameraOptions,
      ...
      if (transform.projection.name === 'globe') {
          return this._cameraForBoundsOnGlobe(
        transform,
        p0,
        p1,
        bearing,
      ...
      const centerOffset = typeof eOptions.offset.x === 'number' &&
      typeof eOptions.offset.y === 'number' ?
  ...
  fitScreenCoordinates(
    ...
    eventData?: Object,
      ...
      const min = new Point(
      Math.min(screen0.x, screen1.x),
      Math.min(screen0.y, screen1.y),
      );
      const max = new Point(
      Math.max(screen0.x, screen1.x),
      Math.max(screen0.y, screen1.y),
      );

      if (
          this.transform.projection.name === 'mercator' &&
        this.transform.anyCornerOffEdge(screen0, screen1)
      ) {
          return this;
      }
      ...
      const cameraPlacement = this._cameraForBounds(
      this.transform,
      p0coord,
      p1coord,
      bearing,
  ...
  _fitInternal(
    calculatedOptions?: ?EasingOptions,
    options?: EasingOptions,
    eventData?: Object,
  ): this {
  ...
  jumpTo(
    options: & CameraOptions
      & { preloadOnly?: $PropertyType<AnimationOptions, 'preloadOnly'> },
    eventData?: Object,
  ): this {
      this.stop();
      ...
      if (zoomChanged) {
          this.fire(new Event('zoomstart', eventData)).fire(
        new Event('zoom', eventData),
          ).fire(new Event('zoomend', eventData));
      }

      if (bearingChanged) {
          this.fire(new Event('rotatestart', eventData)).fire(
        new Event('rotate', eventData),
          ).fire(new Event('rotateend', eventData));
      }

      if (pitchChanged) {
          this.fire(new Event('pitchstart', eventData)).fire(
        new Event('pitch', eventData),
  ...
  flyTo(options: EasingOptions, eventData?: Object): this {
      ...
      /**
         ...
         */
      function r(i) {
          const b = (w1 * w1 - w0 * w0 + (i ? -1 : 1) * rho2 * rho2 * u1 * u1) / (2 * (i ?
              w1 :
              w0) * rho2 * u1);
      ...
      if ('duration' in options) {
          ...
          const V = 'screenSpeed' in options ?
