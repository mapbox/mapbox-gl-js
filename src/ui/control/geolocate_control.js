...
class GeolocateControl
  ...
  _checkGeolocationSupport(callback: (boolean) => void) {
      ...
      // in some environments like iOS16 WebView, permissions reject queries but still support geolocation
          window.navigator.permissions.query({name: 'geolocation'}).then(
        p => updateSupport(p.state !== 'denied'),
          ).catch(() => updateSupport());
      } else {
          updateSupport();
      }
  ...
  _setupUI: ((supported: boolean) => void) = (supported: boolean) => {
      ...
      DOM.create('span', `mapboxgl-ctrl-icon`, this._geolocateButton).setAttribute(
      'aria-hidden',
      'true',
      );
      ...
      if (this.options.showUserLocation) {
        ...
        DOM.create('div', 'mapboxgl-user-location-dot'),
          );
          this._dotElement.appendChild(
        DOM.create('div', 'mapboxgl-user-location-heading'),
          );
        ...
        {element: this._circleElement, pitchAlignment: 'map'},
          );
      ...
      if (this.options.trackUserLocation) {
        ...
        'movestart',
        event => {
            const fromResize = event.originalEvent &&
            event.originalEvent.type === 'resize';
            if (
                !event.geolocateSource && this._watchState === 'ACTIVE_LOCK' &&
              !fromResize
            ) {
                this._watchState = 'BACKGROUND';
                this._geolocateButton.classList.add(
