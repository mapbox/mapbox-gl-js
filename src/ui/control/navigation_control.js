...
class NavigationControl {
  ...
  constructor(options: Options) {
      ...
      if (this.options.showZoom) {
        ...
        e => {
            if (this._map) this._map.zoomIn({}, {originalEvent: e});
        },
          );
          DOM.create('span', `mapboxgl-ctrl-icon`, this._zoomInButton).setAttribute(
        'aria-hidden',
        'true',
          );
        ...
        e => {
            if (this._map) this._map.zoomOut({}, {originalEvent: e});
        },
          );
          DOM.create('span', `mapboxgl-ctrl-icon`, this._zoomOutButton).setAttribute(
        'aria-hidden',
        'true',
          );
      }
  ...
  _rotateCompassArrow = () => {
      ...
      const rotate = this.options.visualizePitch ?
          `scale(${1 / Math.pow(
        Math.cos(map.transform.pitch * (Math.PI / 180)),
        0.5,
          )}) rotateX(${map.transform.pitch}deg) rotateZ(${map.transform.angle * (180 / Math.PI)}deg)` :
...
class MouseRotateWrapper {
  ...
  touchmove = (e: TouchEvent) => {
      if (e.targetTouches.length !== 1) {
          ...
          this.move(
        (({preventDefault: () => e.preventDefault()}: any): MouseEvent),
        this._lastPos,
          );
      }
  };
