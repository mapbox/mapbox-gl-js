...
class ScrollZoomHandler {
  ...
  wheel(e: WheelEvent) {
      ...
      let value = e.deltaMode === (window.WheelEvent: any).DOM_DELTA_LINE ?
          e.deltaY * 40 :
          e.deltaY;
  ...
  renderFrame(): ?HandlerResult {
      ...
      if (
          this._type === 'wheel' && tr.projection.wrap &&
        (tr._center.lng >= 180 || tr._center.lng <= -180)
      ) {
          this._prevEase = null;
          this._easing = null;
      ...
      const startingZoom = (() => {
          return tr._terrainEnabled() && this._aroundCoord ?
              tr.computeZoomRelativeTo(this._aroundCoord) :
              tr.zoom;
      });
      ...
      // For trackpad events and single mouse wheel ticks, use the default zoom rate
          const zoomRate = this._type === 'wheel' &&
        Math.abs(this._delta) > wheelZoomDelta ?
              this._wheelZoomRate :
              this._defaultZoomRate;
          // Scale by sigmoid of scroll wheel delta.
          let scale = maxScalePerFrame / (1 + Math.exp(
        -Math.abs(this._delta * zoomRate),
          ));
          ...
          const fromScale = typeof this._targetZoom === 'number' ?
  ...
  _addScrollZoomBlocker() {
      if (this._map && !this._alertContainer) {
          this._alertContainer = DOM.create(
        'div',
        ...
        10,
        Math.min(24, Math.floor(this._el.clientWidth * 0.05)),
          )}px`;
      }
  }
