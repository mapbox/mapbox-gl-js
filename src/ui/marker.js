...
export default class Marker
  ...
  constructor(options?: Options, legacyOptions?: Options) {
      ...
      this._pitchAlignment = options && options.pitchAlignment &&
      ...
      if (!options || !options.element) {
        ...
        DOM.createSVG('defs', {}, svg),
          ...
          DOM.createSVG(
        'ellipse',
        {cx: 13.5, cy: 34.8, rx: 10.5, ry: 5.25, fill: 'url(#shadowGradient)'},
        svg,
          ); // shadow
  ...
  _onKeyPress: ((e: KeyboardEvent) => void) = (e: KeyboardEvent) => {
      ...
      if (
          code === 'Space' || code === 'Enter' || legacyCode === 32 ||
  ...
  _onMapClick: ((e: MapMouseEvent) => void) = (e: MapMouseEvent) => {
      ...
      if (
          this._popup &&
        (targetElement === element || element.contains((targetElement: any)))
      ) {
          this.togglePopup();
      }
  };
  ...
  _evaluateOpacity: (() => void) = () => {
      ...
      if (
          !pos || pos.x < 0 || pos.x > map.transform.width || pos.y < 0 ||
      ...
      if (map._showingGlobe() && isLngLatBehindGlobe(map.transform, this._lngLat)) {
          ...
          if (
              map.transform._terrainEnabled() && map.getTerrain() &&
          this._behindTerrain()
          ) {
  ...
  _calculateXYTransform(): string {
      ...
      const manhattanDistance = Math.abs(posFromCenter.x) + Math.abs(
      posFromCenter.y,
      );
  ...
  _calculateZTransform(): string {
      ...
      if (alignment === 'map') {
          ...
          new LngLat(this._lngLat.lng, this._lngLat.lat + .001),
              );
  ...
  getRotationAlignment(): string {
      ...
      if (
          this._rotationAlignment === 'horizon' && this._map &&
