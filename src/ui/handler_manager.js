...
class HandlerManager {
  ...
  constructor(
    ...
    options: {
      ...
      for (const [target, type, listenerOptions] of this._listeners) {
          const listener = target === window.document ?
  ...
  handleEvent = (e: InputEvent | RenderFrameEvent, eventName?: string) => {
      ...
      const points = mapTouches ?
          ...
          isRenderFrame ?
              undefined        : // renderFrame event doesn't have any points
      ...
      if (
          Object.keys(deactivatedHandlers).length || hasChange(mergedHandlerResult)
      ) {
          this._changes.push(
        [mergedHandlerResult, eventsInProgress, deactivatedHandlers],
          );
          this._triggerRenderFrame();
      }
    ...
    deactivatedHandlers: Object,
      ...
      if (panDelta) {
          if (tr.projection.name === 'mercator') {
              assert(
          this._dragOrigin,
          '_dragOrigin should have been setup with a previous dragstart',
              );
              const startPoint = this._trackingEllipsoid.projectRay(
          tr.screenPointToMercatorRay(around).dir,
              );
              const endPoint = this._trackingEllipsoid.projectRay(
          tr.screenPointToMercatorRay(around.sub(panDelta)).dir,
              );
              ...
              if (tr.projection.name === 'globe') {
                  ...
                  panVec[0] = -panDelta.x * mercatorScale(
            latFromMercatorY(startPoint.y),
                  ) * scale;
      ...
      // This way the zoom interpolation can be kept linear and independent of the (possible) terrain elevation
          const pickedPosition: Vec3 = aroundCoord ?
              toVec3(aroundCoord) :
              toVec3(tr.pointCoordinate3D(around));

          const aroundRay = {
              dir: vec3.normalize(
          [],
          vec3.sub([], pickedPosition, tr._camera.position),
              ),
          };
    ...
    allowEndAnimation: boolean,
      ...
      if (allowEndAnimation && (wasMoving || nowMoving) && !stillMoving) {
        ...
        this._map.dragPan._inertiaOptions,
          ...
          const shouldSnapToNorth = (bearing => bearing !== 0 &&
  ...
  _requestFrame(): number {
      ...
      return this._map._renderTaskQueue.add(
      timeStamp => {
