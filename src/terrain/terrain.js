...
export class Terrain
  ...
  constructor(painter: Painter, style: Style) {
      ...
      const [triangleGridArray, triangleGridIndices, skirtIndicesOffset] = createGrid(
      GRID_DIM + 1,
      );
      const context = painter.context;
      this.gridBuffer = context.createVertexBuffer(
      triangleGridArray,
      posAttributes.members,
      );
      this.gridIndexBuffer = context.createIndexBuffer(triangleGridIndices);
      this.gridSegments = SegmentVector.simpleSegment(
      0,
      0,
      triangleGridArray.length,
      triangleGridIndices.length,
      );
      ...
      const epsilon = this.painter.transform.projection.name === 'globe' ?
          .015 :
          0; // Experimentally the smallest value to avoid rendering artifacts (https://github.com/mapbox/mapbox-gl-js/issues/11975)
      mat4.ortho(this.orthoMatrix, epsilon, EXTENT, 0, EXTENT, 0, 1);
      const gl = context.gl;
      this._overlapStencilMode = new StencilMode(
  ...
  update(style: Style, transform: Transform, adaptCameraAltitude: boolean) {
      if (style && style.terrain) {
          ...
          this.sourceCache = isDrapeModeDeferred ?
  ...
  updateTileBinding(sourcesCoords: { [string]: Array<OverscaledTileID> }) {
      ...
      // Don't activate terrain until center tile gets loaded.
          this._initializing = tr._centerAltitude === 0 &&
        this.getAtPointOrZero(MercatorCoordinate.fromLngLat(tr.center), -1) === -1;
          this._emptyDEMTextureDirty = !this._initializing;
      }

      const coords = this.proxyCoords = psc.getIds().map(
      id => {
      ...
      this.proxiedCoords[psc.id] = coords.map(
      tileID => new ProxiedTileID(tileID, tileID.key, this.orthoMatrix),
      );
  ...
  _prepareDemTileUniforms(
    ...
    uniformSuffix: ?string,
      ...
      if (!demTile || demTile.demTexture == null) return false;
  ...
  get emptyDepthBufferTexture(): Texture {
      ...
      if (!this._emptyDepthBufferTexture) {
        ...
        Uint8Array.of(255, 255, 255, 255),
          ...
          this._emptyDepthBufferTexture = new Texture(
        context,
        image,
        gl.RGBA,
        {premultiply: false},
  ...
  _getLoadedAreaMinimum(): number {
      let nonzero = 0;
      const min = this._visibleDemTiles.reduce(
      (acc, tile) => {
  ...
  _updateEmptyDEMTexture(): Texture {
      ...
      new Uint8Array(
        DEMData.pack(
          min,
          ((this.sourceCache.getSource(): any): RasterDEMTileSource).encoding,
        ),
      ),
      );
      ...
      if (!texture) {
          texture = this._emptyDEMTexture = new Texture(
        context,
        image,
        gl.RGBA,
        {premultiply: false},
  ...
  setupElevationDraw(
    ...
    options?: {
      ...
      // Both DEM textures are expected to be correctly set if geomorphing is enabled
          ...
          (prevDemTile.demTexture: any).bind(
        gl.NEAREST,
        gl.CLAMP_TO_EDGE,
        gl.NEAREST,
          );
      ...
      if (options && options.useMeterToDem && demTile) {
          const meterToDEM = (1 << demTile.tileID.canonical.z) * mercatorZfromAltitude(
        1,
      ...
      if (this.painter.transform.projection.name === 'globe') {
          const globeUniforms = this.globeUniformValues(
        this.painter.transform,
        tile.tileID.canonical,
        options && options.useDenormalizedUpVectorScale,
          );
  ...
  renderBatch(startLayerIndex: number): number {
      ...
      // bind framebuffer and assign texture to the tile (texture used in drawTerrainRaster).
          ...
          const renderCacheIndex = psc.proxyCachedFBO[proxy.key] ?
              psc.proxyCachedFBO[proxy.key][startLayerIndex] :
              undefined;
          const fbo = renderCacheIndex !== undefined ?
              psc.renderCache[renderCacheIndex] :
              this.pool[poolIndex++];
          ...
          for (let j = drapedLayerBatch.start; j <= drapedLayerBatch.end; ++j) {
              ...
              const proxiedCoords = sourceCache ?
                  this.proxyToSource[proxy.key][sourceCache.id] :
                  [proxy];
              if (!proxiedCoords)
                  continue; // when tile is not loaded yet for the source cache.
  ...
  renderCacheEfficiency(style: Style): Object {
      ...
      return {
          efficiency: (1.0 - uncacheableLayerCount / drapedLayerCount) * 100.0,
          firstUndrapedLayer,
      };
  }
  ...
  getMinElevationBelowMSL(): number {
      ...
      this._visibleDemTiles.filter(tile => tile.dem).forEach(
      tile => {
  ...
  raycast(pos: Vec3, dir: Vec3, exaggeration: number): ?number {
      ...
      const preparedTiles = this._visibleDemTiles.filter(tile => tile.dem).map(
      tile => {
  ...
  _createFBO(): FBO {
      ...
      context,
      {width: bufferSize[0], height: bufferSize[1], data: null},
      gl.RGBA,
      );
      ...
      if (this._sharedDepthStencil === undefined) {
          this._sharedDepthStencil = context.createRenderbuffer(
        context.gl.DEPTH_STENCIL,
        bufferSize[0],
        bufferSize[1],
          );
          this._stencilRef = 0;
      ...
      if (
        ...
        !context.extTextureFilterAnisotropicForceOff
      ) {
          gl.texParameterf(
        gl.TEXTURE_2D,
  ...
  _setupDrapedRenderBatches() {
      ...
      while (
          !this._style.isLayerDraped(layer) &&
        layer.isHidden(this.painter.transform.zoom) &&
  ...
  _setupRenderCache(previousProxyToSource: {[number]: {[string]: Array<ProxiedTileID>}}) {
      ...
      for (let i = coords.length - 1; i >= 0; i--) {
          ...
          if (psc.proxyCachedFBO[proxy.key] !== undefined) {
              ...
              for (const proxyFBO in psc.proxyCachedFBO[proxy.key]) {
                  psc.renderCache[psc.proxyCachedFBO[proxy.key][proxyFBO]].dirty = equal < 0 ||
            equal !== Object.values(prev).length;
              }
          }
      }

      const sortedRenderBatches = [...this._drapedRenderBatches];
      sortedRenderBatches.sort(
      (batchA, batchB) => {
      ...
      for (const batch of sortedRenderBatches) {
          for (const id of coords) {
              ...
              if (
                  index === undefined && psc.renderCache.length < RENDER_CACHE_MAX_SIZE
              ) {
  ...
  _setupStencil(
    fbo: FBO,
    proxiedCoords: Array<ProxiedTileID>,
    layer: StyleLayer,
    sourceCache?: SourceCache,
  ) {
      ...
      if (layer.isTileClipped()) {
          ...
          proxiedCoords[0].overscaledZ > proxiedCoords[proxiedCoords.length - 1].overscaledZ
      ) {
          stencilRange = 1;
  ...
  clipOrMaskOverlapStencilType(): boolean {
      return (
          this._overlapStencilType === 'Clip' || this._overlapStencilType === 'Mask'
      );
  }
  ...
  pointCoordinate(screenPoint: Point): ?Vec4 {
      ...
      if (
          screenPoint.x < 0 || screenPoint.x > transform.width || screenPoint.y < 0 ||
        screenPoint.y > transform.height
      ) {
          return null;
      }
  ...
  drawDepth() {
      ...
      const width = Math.ceil(painter.width),
          height = Math.ceil(painter.height);
      if (
          this._depthFBO &&
        (this._depthFBO.width !== width || this._depthFBO.height !== height)
      ) {
          this._depthFBO.destroy();
          this._depthFBO = undefined;
          this._depthTexture = undefined;
    ...
    previousProxyToSource: { [number]: { [string]: Array<ProxiedTileID> } },
      ...
      for (let i = 0; i < proxys.length; i++) {
          ...
          if (proxied) {
              ...
              const id = this._createProxiedId(
          proxyTileID,
          proxied,
          previousProxyToSource[proxyTileID.key] &&
            previousProxyToSource[proxyTileID.key][sourceCache.id],
              );
              coords.push(id);
      ...
      for (let i = 0; i < sourceCoords.length; i++) {
          ...
          const proxy = this._findTileCoveringTileID(
        tile.tileID,
        this.proxySourceCache,
          );
          ...
          proxy.tileID,
          tile,
          previousProxyToSource[proxy.tileID.key] &&
            previousProxyToSource[proxy.tileID.key][sourceCache.id],
              );
              if (!array) {
  ...
  _setupProxiedCoordsForImageSource(
    sourceCache: SourceCache,
    sourceCoords: Array<OverscaledTileID>,
    previousProxyToSource: { [number]: { [string]: Array<ProxiedTileID> } },
  ) {
      ...
      const anchor = new Point(imageSource.tileID.x, imageSource.tileID.y)._div(
      1 << imageSource.tileID.z,
      );
      const aabb = imageSource.coordinates.map(MercatorCoordinate.fromLngLat).reduce(
      (acc, coord) => {
      ...
      const tileOutsideImage = ((tileID, imageTileID) => {
          ...
          return (
              x + d < ix + aabb.min.x || x > ix + aabb.max.x ||
          y + d < iy + aabb.min.y ||
          y > iy + aabb.max.y
          );
      });
  ...
  _createProxiedId(
    proxyTileID: OverscaledTileID,
    tile: Tile,
    recycle: Array<ProxiedTileID>,
  ): ProxiedTileID {
      ...
      if (!key) {
          ...
          if (tileID.canonical.z >= maxzoom) {
              ...
              if (sourceCache.getSource().reparseOverscaled) {
                  ...
                  sourceTileID = new OverscaledTileID(
            z,
            tileID.wrap,
            maxzoom,
  ...
  findDEMTileFor(tileID: OverscaledTileID): ?Tile {
      return this.enabled ?
          this._findTileCoveringTileID(tileID, this.sourceCache) :
          null;
  }
