...
class SourceCache extends Evented {
  ...
  _loadedSiblingTiles: Partial<
    Record<number | string, Tile | null | undefined>
  >;
  ...
  update(
    ...
    if (
      ...
      if (isBatchedModelType) {
        const batchedModelTileIDs = transform.extendTileCover(
          idealTileIDs,
          idealZoom
        );
        ...
        const shadowCasterTileIDs = transform.extendTileCover(
          idealTileIDs,
          idealZoom,
          directionalLight
        );
    ...
    if (isRasterType(this._source.type) && idealTileIDs.length !== 0) {
      const tilesForFading: Partial<Record<string | number, OverscaledTileID>> =
        {};
      ...
      for (const id of ids) {
        ...
        const siblingTiles = this.findLoadedSibling(tileID);
        const tileForFading = parentTile || siblingTiles;

        if (tileForFading) {
          this._addTile(tileForFading.tileID);
          tilesForFading[tileForFading.tileID.key] = tileForFading.tileID;
        }
    ...
    // Construct a cache of loaded parents and siblings
