...
export default class Worker {
  ...
  constructor(self: WorkerGlobalScopeInterface) {
      ...
      name: string,
      WorkerSource: Class<WorkerSource>,
      ) => {
  ...
  updateLayers(
    mapId: string,
    params: { layers: Array<LayerSpecification>, removedIds: Array<string> },
    callback: WorkerTileCallback,
  ) {
  ...
  loadTile(
    mapId: string,
    params: WorkerTileParameters & { type: string },
    callback: WorkerTileCallback,
  ) {
      assert(params.type);
      const p = this.enableTerrain ?
          extend({enableTerrain: this.terrain}, params) :
          params;
      p.projection = this.projections[mapId] || this.defaultProjection;
      this.getWorkerSource(mapId, params.type, params.source).loadTile(
      p,
      callback,
      );
  }
  ...
  loadDEMTile(
    mapId: string,
    params: WorkerDEMTileParameters,
    callback: WorkerDEMTileCallback,
  ) {
      const p = this.enableTerrain ?
          extend({buildQuadTree: this.terrain}, params) :
          params;
  ...
  reloadTile(
    ...
    callback: WorkerTileCallback,
      ...
      const p = this.enableTerrain ?
          ...
          params;
      p.projection = this.projections[mapId] || this.defaultProjection;
      this.getWorkerSource(mapId, params.type, params.source).reloadTile(
      p,
      callback,
      );
  }
  ...
  abortTile(
    mapId: string,
    params: TileParameters & { type: string },
    callback: WorkerTileCallback,
  ) {
      assert(params.type);
      this.getWorkerSource(mapId, params.type, params.source).abortTile(
      params,
      callback,
      );
  }
  ...
  removeTile(
    ...
    callback: WorkerTileCallback,
      ...
      assert(params.type);
      this.getWorkerSource(mapId, params.type, params.source).removeTile(
      params,
      callback,
      );
  }
  ...
  removeSource(
    ...
    callback: WorkerTileCallback,
      ...
      if (
          !this.workerSources[mapId] || !this.workerSources[mapId][params.type] ||
  ...
  syncRTLPluginState(
    ...
    callback: Callback<boolean>,
      ...
      try {
          ...
          if (
              globalRTLTextPlugin.isLoaded() && !globalRTLTextPlugin.isParsed() &&
