...
class SourceCache extends Evented {
     ...
     */
    getIds(): Array<string> {
        return (Object.values(this._tiles): any).map((tile: Tile) => tile.tileID).sort(compareIdZoom).map(id => id.key);
    }
    ...
    getRenderableIds(symbolLayer?: boolean): Array<string> {
        ...
        return renderables.map(tile => tile.tileID).sort(compareIdZoom).map(id => id.key);
    }
