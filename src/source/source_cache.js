...
class SourceCache
  ...
  constructor(id: string, source: Source, onlySymbols?: boolean) {
      ...
      e => {
          ...
          if (
              this._sourceLoaded && !this._paused && e.dataType === "source" &&
  ...
  onAdd(map: MapboxMap) {
      this.map = map;
      this._minTileCacheSize = this._minTileCacheSize === undefined && map ?
          map._minTileCacheSize :
          this._minTileCacheSize;
      this._maxTileCacheSize = this._maxTileCacheSize === undefined && map ?
  ...
  /**
     ...
     */
  getIds(): Array<number> {
      return values((this._tiles: any)).map((tile: Tile) => tile.tileID).sort(
      compareTileId,
      ).map(id => id.key);
  }
  ...
  getRenderableIds(symbolLayer?: boolean): Array<number> {
      ...
      if (symbolLayer) {
        ...
        (a_: Tile, b_: Tile) => {
            ...
            return (
                a.overscaledZ - b.overscaledZ || rotatedB.y - rotatedA.y ||
              rotatedB.x - rotatedA.x
            );
        },
          ).map(tile => tile.tileID.key);
      }
      return renderables.map(tile => tile.tileID).sort(compareTileId).map(
      id => id.key,
      );
  }
  ...
  _isIdRenderable(id: number, symbolLayer?: boolean): boolean {
      return (
          this._tiles[id] && this._tiles[id].hasData() && !this._coveredTiles[id] &&
  ...
  _tileLoaded: ((tile: Tile, id: number, previousState: TileState, err: ?Error) => void) = (tile: Tile, id: number, previousState: TileState, err: ?Error) => {
      if (err) {
          ...
          if ((err: any).status !== 404)
              this._source.fire(new ErrorEvent(err, {tile})); else {
              // continue to try loading parent/children tiles if a tile doesn't exist (404)
              const updateForTerrain = this._source.type === 'raster-dem' &&
          this.usedForTerrain;
  ...
  _retainLoadedChildren(
    ...
    retain: { [number | string]: OverscaledTileID },
      ...
      for (const id in this._tiles) {
          ...
          if (
              retain[id] || !tile.hasData() || tile.tileID.overscaledZ <= zoom ||
  ...
  updateCacheSize(transform: Transform, tileSize?: number) {
      ...
      const minSize = typeof this._minTileCacheSize === 'number' ?
  ...
  update(transform: Transform, tileSize?: number, updateForTerrain?: boolean) {
      ...
      if (!this.used && !this.usedForTerrain) {
          ...
          idealTileIDs = transform.getVisibleUnwrappedCoordinates(
        this._source.tileID,
          ).map(
          ...
          idealTileIDs = transform.coveringTiles(
        {
          ...
          if (this._source.hasTile) {
              idealTileIDs = idealTileIDs.filter(
          coord => (this._source.hasTile: any)(coord),
              );
          }
      }
    ...
    idealTileIDs: Array<OverscaledTileID>,
  ): { [_: number | string]: OverscaledTileID } {
      ...
      for (const tileID of idealTileIDs) {
          ...
          if (tileID.canonical.z >= this._source.maxzoom) {
              ...
              if (
                  retain[children[0].key] && retain[children[1].key] &&
  ...
  _addTile(tileID: OverscaledTileID): Tile {
      ...
      if (!cached)
        ...
        new Event(
          'dataloading',
          {tile, coord: tile.tileID, dataType: 'source'},
        ),
          );
  ...
  getVisibleCoordinates(symbolLayer?: boolean): Array<OverscaledTileID> {
      const coords = this.getRenderableIds(symbolLayer).map(
  ...
  _preloadTiles(
    ...
    callback: Callback<any>,
      ...
      const terrain = this.map.painter.terrain;
      const tileSize = this.usedForTerrain && terrain ?
