...
class Style
  ...
  setState(nextState: StyleSpecification): boolean {
      ...
      if (unimplementedOps.length > 0) {
          throw (
              new Error(`Unimplemented: ${unimplementedOps.map(op => op.command).join(
            ', ',
              )}.`,)
          );
      }
  ...
  addSource(
    ...
    options: StyleSetterOptions = {},
      ...
      if (!source.type) {
          throw (
              new Error(`The type property must be defined, but only the following properties were given: ${Object.keys(
            source,
              ).join(', ')}.`,)
          );
      }
      ...
      sourceInstance.setEventedParent(
      this,
      () => ({
  ...
  removeSource(id: string): this {
      ...
      for (const layerId in this._layers) {
          if (this._layers[layerId].source === id) {
              return this.fire(
          new ErrorEvent(
            new Error(`Source "${id}" cannot be removed while layer "${layerId}" is using it.`,),
          ),
              );
          }
      }
      if (this.terrain && this.terrain.get().source === id) {
          return this.fire(
        new ErrorEvent(
          new Error(`Source "${id}" cannot be removed while terrain is using it.`,),
        ),
          );
      }
  ...
  addLayer(
    ...
    options: StyleSetterOptions = {},
      ...
      const sourceCache = this._getLayerSourceCache(layer);
      if (
          this._removedLayers[id] && layer.source && sourceCache &&
        layer.type !== 'custom'
      ) {
  ...
  moveLayer(id: string, before?: string) {
      ...
      if (!layer) {
          this.fire(
        new ErrorEvent(
          new Error(`The layer '${id}' does not exist in the map's style and cannot be moved.`,),
        ),
          );
          return;
      }
  ...
  removeLayer(id: string) {
      ...
      if (!layer) {
          this.fire(
        new ErrorEvent(
          new Error(`The layer '${id}' does not exist in the map's style and cannot be removed.`,),
        ),
          );
          return;
      }
  ...
  setFilter(
    ...
    options: StyleSetterOptions = {},
      ...
      if (!layer) {
          this.fire(
        new ErrorEvent(
          new Error(`The layer '${layerId}' does not exist in the map's style and cannot be filtered.`,),
        ),
          );
          return;
      }
  ...
  setLayoutProperty(
    ...
    options: StyleSetterOptions = {},
      ...
      if (!layer) {
          this.fire(
        new ErrorEvent(
          new Error(`The layer '${layerId}' does not exist in the map's style and cannot be styled.`,),
        ),
          );
          return;
      }
  ...
  setFeatureState(
    ...
    state: Object,
      ...
      if (!source) {
          this.fire(
        new ErrorEvent(
          new Error(`The source '${sourceId}' does not exist in the map's style.`,),
        ),
          );
          return;
      }
  ...
  _updateLayer(layer: StyleLayer) {
      ...
      const sourceCache = this._getLayerSourceCache(layer);
      if (
          layer.source && !this._updatedSources[layer.source] &&
      ...
      if (params && params.layers) {
          ...
          for (const layerId of params.layers) {
              ...
              if (!layer) {
                  ...
                  this.fire(
            new ErrorEvent(
              new Error(`The layer '${layerId}' does not exist in the map's style and cannot be queried for features.`,),
            ),
                  );
                  return [];
              }
  ...
  querySourceFeatures(
    sourceID: string,
    params: ?{ sourceLayer: ?string, filter: ?Array<any>, validate?: boolean },
  ): Array<QueryFeature> {
      if (params && params.filter) {
          this._validate(
        validateFilter,
    ...
    drapeRenderMode: number = DrapeRenderMode.elevated,
      ...
      // Updating
          ...
          for (const name of Object.keys(styleSpec.terrain)) {
              ...
              if (
                  !terrainOptions.hasOwnProperty(name) &&
  ...
  _setTransitionParameters(transitionOptions: Object): TransitionParameters {
      return {
          now: browser.now(),
          transition: extend(transitionOptions, this.stylesheet.transition),
      };
  }
  ...
  _updateDrapeFirstLayers() {
      ...
      const draped = this._order.filter(
      id => {
          return this.isLayerDraped(this._layers[id]);
      },
      );

      const nonDraped = this._order.filter(
      id => {
          return !this.isLayerDraped(this._layers[id]);
      },
      );
    ...
    forceFullPlacement: boolean = false,
      ...
      for (const layerID of this._order) {
          ...
          if (!layerTiles[styleLayer.source]) {
              ...
              layerTiles[styleLayer.source] = sourceCache.getRenderableIds(true).map(
          id => sourceCache.getTileByID(id),
              ).sort(
          (a, b) => b.tileID.overscaledZ - a.tileID.overscaledZ ||
      ...
      const needsRerender = !this.pauseablePlacement.isDone() ||
