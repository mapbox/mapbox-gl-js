...
function projectTruncatedLineSegment(previousTilePoint: Point, currentTilePoint: Point, previousProjectedPoint: Point, minimumLength: number, projectionMatrix: mat4, getElevation: ?((p: Point) => Array<number>), projection: Projection, tileID: CanonicalTileID) {
    ...
    //const projectedUnitVertex2 = projectToGlobe(unitVertex, projectionMatrix, getElevation ? getElevation(unitVertex) : [0, 0, 0], tileID).point;
...
function placeGlyphAlongLine(offsetX: number,
    ...
    while (distanceToPrev + currentSegmentDistance <= absOffsetX) {
        ...
        if (current === undefined) {
            ...
            //const projection2 = projectToGlobe(currentVertex, labelPlaneMatrix, getElevation ? getElevation(currentVertex) : [0, 0, 0], tileID);
