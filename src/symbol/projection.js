...
function placeFirstAndLastGlyph(fontScale: number, glyphOffsetArray: any, lineOffsetX: number, lineOffsetY: number, flip: boolean, anchorPoint: Point, symbol: any, lineVertexArray: any, labelPlaneMatrix: mat4, projectionCache: any, returnTileDistance: boolean) {
...
function placeGlyphsAlongLine(symbol, fontSize, flip, keepUpright, posMatrix, labelPlaneMatrix, glCoordMatrix, glyphOffsetArray, lineVertexArray, dynamicLayoutVertexArray, anchorPoint, projectionCache) {
    ...
    if (symbol.numGlyphs > 1) {
        ...
        if (keepUpright && !flip) {
            const a = project((lineVertexArray.get(symbol.lineStartIndex + symbol.segment): any), posMatrix);
            const b = project((lineVertexArray.get(symbol.lineStartIndex + symbol.segment + 1): any), posMatrix);
