...
function updateBorders(context: Context, source: SourceCache, coord: OverscaledTileID, bucket: FillExtrusionBucket, layer: FillExtrusionStyleLayer, terrain: Terrain, reconcileReplacementState) {
    ...
    const flatBase = (min: number, max: number, edge: number, neighborDEMTile, neighborTileID, verticalEdge: boolean, maxOffsetFromBorder) => {
        if (!terrain) {
            return 0;
        }
