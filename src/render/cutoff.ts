...
export const getCutoffParams = (painter: Painter, cutoffFadeRange: number): CutoffParams => {
    ...
    if (pitch < activationThreshold - 15) {
        return {
            shouldRenderCutoff: false,
            uniformValues: {
                'u_cutoff_params': [0, 0, 0, 1]
    ...
    const fadeRangePixels = cutoffFadeRange * tr.height * 1.3; // slight adjustment to make it more prominent

    // Half-rate exponential zoom scaling: grows with zoom but stays bounded
    const zoomScale = Math.pow(2.0, Math.max(tr._zoom - painter.minCutoffZoom, 0.0) * 0.85);
    const cutoffDistance = tr.cameraToCenterDistance * 1.4 * zoomScale;

    // Smooth activation over pitch range
    const activation = smoothstep(activationThreshold - 15, activationThreshold, pitch);
